...
class Player:
    ...
    def nextMove(self, state):
        ...
        if len(validMoves) != 0:
            ...
            for i in range(0, 10):
                ...
                bestMove = max(move, bestMove, key=lambda x: self.alphaBetaPrunning(2, x, state, True, -99999999, +99999999))
            result = bestMove
        return result
    ...
    def alphaBetaPrunning(self, depth, move, state, isMax, alpha, beta):
        lState = copy.deepcopy(state)
        opponent = ''
        possibleMoves = []
        if self.str == 'w':
        ...
        if isMax:
            self.move(move, lState)
            possibleMoves = self.getAllValidMove(lState)
        else:
            opponent.move(move, lState)
            possibleMoves = opponent.getAllValidMove(lState)

        if len(possibleMoves) == 0 or depth == 0:
            if isMax:
                return self.evaluate(move, state)
            else:
                return opponent.evaluate(move, state)

        if isMax:
            value = -99999999 # -INF
            for i in range(0, 2):
                lMove = possibleMoves[random.randint(0, len(possibleMoves) - 1)]
                value = max(value, self.alphaBetaPrunning(depth-1, lMove, lState, not isMax, alpha, beta))
            ...
            for i in range(0, 2):
                lMove = possibleMoves[random.randint(0, len(possibleMoves) - 1)]
                value = min(value, self.alphaBetaPrunning(depth-1, lMove, lState, isMax, alpha, beta))
                beta = min(beta, value)
                if beta <= alpha:
                    break
            return value
